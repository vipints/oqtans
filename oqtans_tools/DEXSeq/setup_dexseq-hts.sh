...
echo Path to HTSeq library files \(PYTHONPATH env variable\) \(currently set to \"$PYTHONPATH\", system version is used if left empty\)
read PYTHONPATH
...
echo
echo generating config file bin/dexseq_config.sh
